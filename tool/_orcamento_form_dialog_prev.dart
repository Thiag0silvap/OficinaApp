...
class _OrcamentoFormDialogState extends State<OrcamentoFormDialog> {
  ...
  void initState() {
    ...
    // Ô£à Preenche dados na edi├º├úo e resolve cliente/ve├¡culo usando provider depois do build
  ...
  Widget build(BuildContext context) {
    ...
    final String itemLabel = _isPecaSelected ? 'Pe├ºa' : 'Servi├ºo';

    final isEdit = widget.orcamentoEditar != null;

    final dialog = ResponsiveDialog(
      title: isEdit ? 'Editar Or├ºamento' : 'Novo Or├ºamento',
      content: Form(
        key: _formKey,
            ...
            child: Column(
              ...
              children: [
                    ...
                    ? Column(
                        children: [
                          ...
                          DropdownButtonFormField<Veiculo>(
                            ...
                            validator: (v) =>
                                v == null ? 'Selecione um ve├¡culo' : null,
                            hint: Text(
                                ...
                                if (!_isPecaSelected)
                                  Expanded(
                                    ...
                                    child: DropdownButtonFormField<String>(
                                      ...
                                      onChanged: (v) =>
                                          setState(() => _pecaSelecionada = v),
                                      hint: const Text('Pe├ºa (opcional)'),
                                    ),
                                  ),
                              ],
                            ),
                  ...
                  if (_aplicarDesconto)
                    TextFormField(
                      ...
                      validator: (v) {
                        ...
                        if (parsed < 0) return 'Desconto inv├ílido';
                ...
                TextFormField(
                  ...
                  decoration: formFieldDecoration(
                    label: 'Observa├º├Áes do Or├ºamento',
      ...
      actions: [
        ...
        ElevatedButton(
          ...
          child: Text(isEdit ? 'Salvar' : 'Gerar Or├ºamento'),
        ),
      ],
    );
  ...
  void _adicionarItem() {
    if (_servicoSelecionado == null || _valorItemController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Preencha o servi├ºo e o valor')),
      );
      return;
    }

    // Ô£à parse correto (R$ 1.234,56)
    final valor = _parseCurrency(_valorItemController.text);
    if (valor == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Valor inv├ílido')));
      return;
    }
  ...
  void _salvarOrcamento() {
    ...
    if (_selectedCliente == null || _selectedVeiculo == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Selecione cliente e ve├¡culo')),
      );
      return;
    }
    ...
    if (_itens.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Adicione pelo menos um item ao or├ºamento'),
        ),
      );
      return;
    }
    ...
    {
      // Evita duplicar a linha "Respons├ível:" a cada edi├º├úo/salvamento.
      final cleanedLines = currentObs
          .split('\n')
          .map((l) => l.trimRight())
          .where((l) => l.trim().isNotEmpty)
          .where((l) => !l.trimLeft().startsWith('Respons├ível:'))
          .toList();

      if (responsavel != null && responsavel.trim().isNotEmpty) {
        cleanedLines.add('Respons├ível: ${responsavel.trim()}');
      }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Or├ºamento salvo com sucesso!'),
